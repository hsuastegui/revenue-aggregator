...
it("renders rows with product name", async () => {
  const app = mount(<App />);
  await flushRequestsAndUpdate(app);

  expect(app.find("table tbody tr").at(56).find("td").first().text()).toEqual(
    "Hominy"
  );
  expect(app.find("table tbody tr").at(73).find("td").first().text()).toEqual(
    "Lychee"
  );
});
...
it("renders table that is sorted ascending", async () => {
  ...
  expect(app.find("table tbody tr").first().find("td").first().text()).toEqual(
    "Alfalfa Sprouts"
  );
  expect(app.find("table tbody tr").last().find("td").first().text()).toEqual(
    "Zucchini Squash"
  );
});
